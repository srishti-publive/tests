...
def push_message(session_id: str, message: dict, maxsize: int, timeout: float = 5.0) -> bool:
    ...
    if the queue is at `maxsize`. Returns False on persistent overflow — the
    caller logs it and records `Custom/MCP/queue_overflow_count`, exactly like
    the `queue.Full` path it replaces. The 5 s default keeps a worker thread from
    being held long by a dead client's full queue.
