...
def get_publisher_data(credentials: dict, args: dict):
    try:
        ...
        if is_missing:
            ...
            logger.warning("get_publisher_data: /publisher-data/ unavailable for publisher=%s — falling back to /footer/", publisher_id)
...
def get_navbar(credentials: dict, args: dict):
    return cds_get(credentials, "/navbar/")


def get_footer(credentials: dict, args: dict):
    return cds_get(credentials, "/footer/")


def get_newsletter_groups(credentials: dict, args: dict):
    try:
...
HANDLERS = {
    "get_publisher_data":    get_publisher_data,
    "get_navbar":            get_navbar,
    "get_footer":            get_footer,
    "get_newsletter_groups": get_newsletter_groups,
}
