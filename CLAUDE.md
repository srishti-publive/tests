...
2. Workers now default to 2, tunable via `WEB_CONCURRENCY` (threads via `GUNICORN_THREADS`, default 4) — set in `entrypoint.sh`, overridable from Railway without an image rebuild. The architecture no longer requires one worker — sessions, queues, stats, and rate limits are all cross-process (Redis) now. Per the staged-rollout plan, raise `WEB_CONCURRENCY` incrementally while watching MCPSessionMissing and queue_overflow_count; set it back to 1 to revert instantly. Running multiple *replicas* (separate containers) additionally requires an external shared `REDIS_URL` — the in-container Redis is per-container.