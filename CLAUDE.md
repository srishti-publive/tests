...
2. Workers default to 2, tunable via `WEB_CONCURRENCY` (threads via `GUNICORN_THREADS`, default 4) — set in `entrypoint.sh`, overridable from Railway without an image rebuild. The architecture no longer requires one worker — sessions, queues, stats, and rate limits are all cross-process via the database now, so multiple workers *and* multiple replicas (separate containers) all share the same state through `DATABASE_URL`.